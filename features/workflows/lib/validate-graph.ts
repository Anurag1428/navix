...
// Structural problems knowable before a run - empty array means runnable. Pure (no db import) so the client can pre-flight the in-hand graph and 
// toast, while the server reuses it as the save-time backstop.
export function validateGraph({ nodes, edges }: WorkflowGraph): string[] {
    ...
    if (triggers !== 1) {
        problems.push(`Graph must have exactly one trigger node, but has ${triggers}.`)
    }
    ...
    if (edges.length === 0) {
        ...
        try {
            ...
            problems.push("Workflow has a cycle -- remove the loop before runnign.")
        }
    }
